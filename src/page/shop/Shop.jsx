...
import {
  addToCartThunk,
  getProductsThunk /*getCartListThunk*/,
} from "../../redux/actions";
...
const Shop = () => {
  ...
  useEffect(() => {
    if (productId) {
      axios
        .get(
          `https://ecommerce-api-react.herokuapp.com/api/v1/products?category=${productId?.category.id}`
        )
        .then((res) => {
      ...
      <div className="components-product">
        ...
        <div className="item-container">
          {productFilter?.map((productItem) => (
            <div key={productItem.id} className="item-product">
